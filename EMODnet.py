...
def gather_depth_data():
    ...
    def get_depth_line(longitude_start, longitude_end, latitude):
        """
    ...
    def process_latitude(lat):
        nonlocal total_data_size_mb
        if total_data_size_mb >= data_limit_mb:
            return None
        depth_profile, response_size_mb = get_depth_line(lon_min, lon_max, lat)
        total_data_size_mb += response_size_mb
    ...
    with ThreadPoolExecutor() as executor:
        results = executor.map(process_latitude, latitudes)
